...
Get_IO_sensitive <- function( meta_res ){
				...
				if( ifelse( is.null( nrow(data[ rownames(data) %in% sig,]) ) , 1 , nrow(data[ rownames(data) %in% sig,]) ) / length( sig ) > 0.7 & ncol(data) > 20 ){
					...
					if( length( geneSig[ !is.na( phenoData(expr[[i]])$os[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] ) ] ) >= 20 ){
						...
						hr = Get_HR_dicho( surv=phenoData(expr[[i]])$os[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] , time=phenoData(expr[[i]])$t.os[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] ,
												 ...
												 title = paste( study[i] , tumor[j] , 36 , "OS_Overall" , sep="_" ) , ylab="Overall Survival" , dir= "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO/IO_Sensitive/KMPlot/OS" )
					...
					if( length( geneSig[ !is.na( phenoData(expr[[i]])$event_occured[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] ) ]  ) >= 20 ){
						...
						hr = Get_HR_dicho( surv=phenoData(expr[[i]])$event_occured[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] , time=phenoData(expr[[i]])$survival_time[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] ,
												 ...
												 title = paste( study[i] , tumor[j] , 36 , "PFS_Overall" , sep="_" ) , ylab="Progression-Free Survival" , dir= "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO/IO_Sensitive/KMPlot/PFS" )
	...
	save( log_response , log_dicho_response , log_tertile_response , file= "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO/IO_Sensitive/IO_Sensitive_LogReg_result.RData" )
	save( cox_os , cox_pfs , cox_dicho_os , cox_dicho_pfs , cox_tertile_os , cox_tertile_pfs , file= "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO/IO_Sensitive/IO_Sensitive_COX_result.RData" ) 
	save(  di_os , di_pfs , file= "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO/IO_Sensitive/IO_Sensitive_DI_result.RData" ) 
}