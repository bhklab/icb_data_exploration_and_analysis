...
for( i in 1:length(study)){
	...
	primary = names( table( phenoData( expr[[i]] )$cancer_type )[ table( phenoData( expr[[i]] )$cancer_type ) >= 20 ] )
...
genes = names( rowSums( numberG )[ rowSums( numberG ) %in% c(7:12) ] ) ## feel free to change 7:12 to get the common genes
  ...
  primary = names( table( phenoData( expr[[i]] )$cancer_type )[ table( phenoData( expr[[i]] )$cancer_type ) >= 20 ] )
	...
	if( length(primary) ){
		for( j in 1:length(primary)){
			...
			data = exprs(expr[[i]])[ ,  phenoData(expr[[i]])$cancer_type %in% primary[j] ]
			remove <- rem(data)
			if( length(remove) ){
				data <- data[-remove,]
			}
			...
			if( nrow(data) & !( primary %in% "Lymph_node" ) ){
				for( k in 1:nrow(data) ){
					...
					x = ifelse( phenoData(expr[[i]])$response[ phenoData(expr[[i]])$cancer_type %in% primary[j] ] %in% "R" , 0 , 
						ifelse( phenoData(expr[[i]])$response[ phenoData(expr[[i]])$cancer_type %in% primary[j] ] %in% "NR" , 1 , NA ) )
...
for( i in 1:length(study)){
	...
	if( length(primary) > 0 ){
		for( j in 1:length(primary)){
			...
			data = as.matrix( data[ rownames(data) %in% genes , ] )
			if( nrow(data) & !( primary %in% "Lymph_node" ) & length( phenoData(expr[[i]])$os[ !is.na( phenoData(expr[[i]])$os ) & phenoData(expr[[i]])$cancer_type %in% primary[j] ] ) >= 20 ){
				for( k in 1:nrow(data) ){
					...
					cox = Get_HR_continous( surv = phenoData(expr[[i]])$os[ phenoData(expr[[i]])$cancer_type %in% primary[j] ] , 
											time = phenoData(expr[[i]])$t.os[ phenoData(expr[[i]])$cancer_type %in% primary[j] ] ,
...
for( i in 1:length(study)){
	...
	if( length(primary) > 0 ){
		for( j in 1:length(primary)){
			...
			data = as.matrix( data[ rownames(data) %in% genes , ] )
			if( nrow(data) & !( primary %in% "Lymph_node" ) & length( phenoData(expr[[i]])$event_occured[ !is.na( phenoData(expr[[i]])$event_occured ) & phenoData(expr[[i]])$cancer_type %in% primary[j] ] ) >= 20 ){
				for( k in 1:nrow(data) ){
					...
					cox = Get_HR_continous( surv = phenoData(expr[[i]])$event_occured[ phenoData(expr[[i]])$cancer_type %in% primary[j] ] , 
											time = phenoData(expr[[i]])$survival_time[ phenoData(expr[[i]])$cancer_type %in% primary[j] ] ,
