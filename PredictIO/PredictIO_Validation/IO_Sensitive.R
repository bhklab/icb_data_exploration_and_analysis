...
Get_IO_Sensitive <- function( exprID , meta_res ){
	...
	if( ifelse( is.null( nrow(data[ rownames(data) %in% sig,]) ) , 1 , nrow(data[ rownames(data) %in% sig,]) ) / length( sig ) > 0.7 & ncol(data) > 10 ){
		...
		if( length( geneSig[ !is.na( clin$os ) ] ) >= 10 ){
			...
			cci = Get_DI_continous( surv=clin$os , time=clin$t.os ,
									 time_censor=36 , variable= geneSig )
			di_os = rbind( di_os , 
					c( exprID , cancer_type , "TPM" , length( geneSig[ !is.na( clin$os ) ] ) ,  cci ) )
			...
			hr = Get_HR_dicho( surv=clin$os , time=clin$t.os ,
									 ...
									 title = paste( exprID , 36 , "OS_Median" , sep="_" ) , ylab="Overall Survival" , dir= paste( "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO_Validation/" , exprID , "/IO_Sensitive" , sep="" ) )
		...
		if( length( geneSig[ !is.na( clin$event_occured ) ]  ) >= 10 ){
			...
			hr = Get_HR_dicho( surv=clin$event_occured , time=clin$survival_time ,
									 ...
									 title = paste( exprID , 36 , "PFS_Median" , sep="_" ) , ylab="Progression-Free Survival" , dir= paste( "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO_Validation/" , exprID  , "/IO_Sensitive" , sep="" ) )
		...
		if( length( geneSig[ !is.na( clin$response ) ]  ) >= 10 ){
			...
			pdf( paste( "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO_Validation/" , exprID , "/IO_Sensitive/ICB_Response.pdf" , sep="" ) , height=3.5,width=3,bg="transparent")
			...
			ggsave(filename= paste( "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO_Validation/" , exprID , "/IO_Sensitive/ICB_Response_ROC.pdf" , sep="" ), plot=ROCPlot, device="pdf", height=4 , width=4 )
	...
	save( log_response , log_dicho_response , file=paste( "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO_Validation/" , exprID , "/IO_Sensitive/IO_Sensitive_LogReg_result.RData" , sep="" ) )
	save( cox_os , cox_pfs , cox_dicho_os , cox_dicho_pfs , file=paste( "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO_Validation/" , exprID , "/IO_Sensitive/IO_Sensitive_COX_result.RData" , sep="" ) ) 
	save(  di_os , di_pfs , file=paste( "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO_Validation/" , exprID , "/IO_Sensitive/IO_Sensitive_DI_result.RData" , sep="" ) ) 
	save(  auc , file=paste( "/home/bioinf/bhklab/farnoosh/PredictIO/Result/PredictIO_Validation/" , exprID , "/IO_Sensitive/IO_Sensitive_AUC.RData" , sep="" ) ) 
}