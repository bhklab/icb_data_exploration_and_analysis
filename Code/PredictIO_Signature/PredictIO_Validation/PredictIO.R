...
Get_PredictIO <- function( exprID , meta_res ){
  ...
  source('~/Code/PredictIO_Signature/meta/Get_DI.R')
	...
	if( !is.null( IOscore ) ){
		if( length( IOscore[ !is.na( clin$os ) ] ) >= 10 ){
			...
			hr = Get_HR_dicho( surv=clin$os , time=clin$t.os ,
									 ...
									 title = paste( exprID , 36 , "OS_Median" , sep="_" ) , ylab="Overall Survival" , dir= paste( "~/Result/PredictIO_Validation/" , exprID , "/PredictIO" , sep="" ) )
		...
		if( length( IOscore[ !is.na( clin$event_occured ) ] ) >= 10 ){
			...
			hr = Get_HR_dicho( surv=clin$event_occured , time=clin$survival_time ,
									 ...
									 title = paste( exprID , 36 , "PFS_Median" , sep="_" ) , ylab="Progression-Free Survival" , dir= paste( "~/Result/PredictIO_Validation/" , exprID , "/PredictIO" , sep="" ) )
		...
		if( length( IOscore[ !is.na( clin$response ) ]  ) >= 10 ){
			...
			pdf( paste( "~/Result/PredictIO_Validation/" , exprID , "/PredictIO/ICB_Response.pdf" , sep="" ) , height=3.5,width=3,bg="transparent")
			...
			ggsave(filename= paste( "~/Result/PredictIO_Validation/" , exprID , "/PredictIO/ICB_Response_ROC.pdf" , sep="" ), plot=ROCPlot, device="pdf", height=4 , width=4 )
	...
	save( log_response , log_dicho_response , file=paste( "~/Result/PredictIO_Validation/" , exprID , "/PredictIO/PredictIO_LogReg_result.RData" , sep="" ) )
	save( cox_os , cox_pfs , cox_dicho_os , cox_dicho_pfs , file=paste( "~/Result/PredictIO_Validation/" , exprID , "/PredictIO/PredictIO_COX_result.RData" , sep="" ) ) 
	save(  di_os , di_pfs , file=paste( "~/Result/PredictIO_Validation/" , exprID , "/PredictIO/PredictIO_DI_result.RData" , sep="" ) ) 
	save(  auc , file=paste( "~/Result/PredictIO_Validation/" , exprID , "/PredictIO/PredictIO_AUC.RData" , sep="" ) ) 
}