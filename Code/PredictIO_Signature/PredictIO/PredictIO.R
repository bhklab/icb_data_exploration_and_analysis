	...
	for( i in 1:length(study)){
		...
		if( length(tumor) > 0 ){
			for( j in 1:length(tumor)){
				...
				if( !is.null( IOscore ) ){
					if( length( IOscore[ !is.na( phenoData(expr[[i]])$os[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] ) ] ) >= 20 ){
						...
						cci = Get_DI_continous( surv=phenoData(expr[[i]])$os[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] , time=phenoData(expr[[i]])$t.os[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] ,
												 time_censor=36 , variable= IOscore )
						di_os = rbind( di_os , 
								c( study[i] , tumor[j] , toupper( phenoData(expr[[i]])$rna[1] ) , length( IOscore[ !is.na( phenoData(expr[[i]])$os ) & phenoData(expr[[i]])$cancer_type %in% tumor[j] ] ) ,  cci ) )
					...
					if( length( IOscore[ !is.na( phenoData(expr[[i]])$event_occured[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] ) ]  ) >= 20 ){
						...
						cci = Get_DI_continous( surv=phenoData(expr[[i]])$event_occured[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] , time=phenoData(expr[[i]])$survival_time[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] ,
												 time_censor=24 , variable= IOscore )
						di_pfs = rbind( di_pfs , 
								c( study[i] , tumor[j] , toupper( phenoData(expr[[i]])$rna[1] ) , length( IOscore[ !is.na( phenoData(expr[[i]])$event_occured ) & phenoData(expr[[i]])$cancer_type %in% tumor[j] ] ) ,  cci ) )
					...
					if( length( IOscore[ !is.na( phenoData(expr[[i]])$response[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] ) ]  ) >= 20 ){
						...
						log_response = rbind( log_response , c( study[i] , 
									...
									length( IOscore[ !is.na( phenoData(expr[[i]])$response[ phenoData(expr[[i]])$cancer_type %in% tumor[j] ] ) ]  ) , 
